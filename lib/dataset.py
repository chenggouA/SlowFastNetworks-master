...
class Base_handle:
    ...
    def __init__(self, clip_len, frame_sample_rate) -> None:
    ...
    def padding(self, buffer, padding):
        
        if padding == "Random":
            # 随机填充
            while buffer.shape[0] < self.clip_len + 2:
                start = np.random.randint(0, buffer.shape[0] + 1)
                padding = buffer[start: , ...]
                buffer = np.vstack((buffer, padding))

        elif padding == "Zero":
            # 零填充
            timestep = buffer.shape[0]
            if timestep < self.clip_len + 2:
                padding = np.zeros((self.clip_len + 2 - timestep, buffer.shape[1], buffer.shape[2], buffer.shape[3]),
                                   dtype=np.float32)
                buffer = np.vstack((buffer, padding))
...
class VideoDataset(Dataset, Base_handle):

    def __init__(self, directory, mode='train', clip_len=8, frame_sample_rate=1, padding = None):
        Base_handle.__init__(self, clip_len, frame_sample_rate)
    ...
    def __getitem__(self, index):
        ...
        if buffer.shape[0] < self.clip_len + 2:
            if self.padding_method == None:
                ...
                buffer = self.padding(buffer, self.padding_method)
...
class Video_handle(Base_handle):
    def __init__(self, video_file, clip_len=8, frame_sample_rate=1, padding = "Zero"):
        Base_handle.__init__(self, clip_len, frame_sample_rate)
        self.padding_method = padding
        self.video_file = video_file

    def get_values(self):
        buffer = self.loadvideo(self.video_file)
    ...
    def set_vido_path(self, video_path):
        self.video_file = video_path


if __name__ == '__main__':

    for i, j in VideoDataset("./dataset", mode="validation", clip_len=128, padding="Zero"):
        print(i, j)